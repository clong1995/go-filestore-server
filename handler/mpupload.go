...
type MultipartUploadInfo struct {
	...
	UploadID   string `json:"upload_id"`  // 分块id
	ChunkSize  int    `json:"chunk_size"` // 分块大小
	ChunkCount int    `json:"chunk"`      // 分块数量
}
...
func InitialMultipartUploadHandler(c *gin.Context) {
	...
	rConn.Do("HSET_", "MP_"+upInfo.UploadID, "filehash", upInfo.FileHash)
	rConn.Do("HSET_", "MP_"+upInfo.UploadID, "filesize", upInfo.FileSize)
...
func UploadPartHandler(w http.ResponseWriter, r *http.Request) {
	...
	fd, err := os.Create(fpath)
	if err != nil {
		w.Write(util.NewRespMsg(-1, "upload path failed", nil).JSONBytes())
	}
	defer fd.Close()
